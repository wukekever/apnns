...
class Parity(object):
    def __init__(self, config, sol, name="Parity_Eqn", **kwargs):
        ...
        self.zmin = config["physical_config"]["z_range"][0]
        self.zmax = config["physical_config"]["z_range"][1]
        ...
        self.num_sim = config["physical_config"]["num_path"]
    ...
    def residual(self, sol, inputs):
        ...
        values, derivatives = self.value_and_grad(sol=sol, inputs=[z, t, x, v])
        ...
        sigma_s = 1.0 + 0.1 * torch.prod(torch.sin(torch.pi*z), axis = -1) 

        res_parity_1 = self.kn ** 2 * (dr_dt + v * dj_dx) - (rho - r) * sigma_s
        res_parity_2 = self.kn ** 2 * dj_dt + v * dr_dx + j * sigma_s
    ...
    def value_and_grad(self, sol, inputs):
        ...
        rho = self.model_rho(net_rho, [z, t, x])
        r = self.model_r(net_r, [z, t, x, v])
        j = self.model_j(net_j, [z, t, x, v])
        ...
        """trick:
            <v j_x> = <vj>_x
        """
        avg_1, avg_2 = self.average_op(net_r=net_r, net_j=net_j, z_t_x=[
                                       z, t, x], vwquads=[self.vquads, self.wquads])
    ...
    def average_op(self, net_r, net_j, z_t_x, vwquads):
        ztx = torch.cat(z_t_x, -1)[:, None, :]
        v, w1 = vwquads
        w2 = w1 * v
        mult_fact = torch.ones((ztx.shape[0], v.shape[0], 1)).to(self.device)

        j_1 = net_j(torch.cat([ztx * mult_fact, v[..., None] * mult_fact], -1))
        j_2 = net_j(
            torch.cat([ztx * mult_fact, - v[..., None] * mult_fact], -1))
        gn = j_1 - j_2
        avg_1 = torch.sum(gn * w2[..., None], dim=-2)

        r_1 = net_r(torch.cat([ztx * mult_fact, v[..., None] * mult_fact], -1))
        r_2 = net_r(
            torch.cat([ztx * mult_fact, - v[..., None] * mult_fact], -1))
    ...
    def bc(self, net_r, net_j, inputs):
        zbc, tbc, vbc_l, vbc_r = inputs
        # Left
        bc_l = [zbc, tbc, self.xmin * torch.ones_like(tbc), vbc_l]
        fbc_l = self.model_r(net_r, bc_l) + self.kn * self.model_j(net_j, bc_l)
        # Right
        bc_r = [zbc, tbc, self.xmax * torch.ones_like(tbc), vbc_r]
    ...
    def ic(self, net_rho, net_r, net_j, inputs):
        zic, xic, vic = inputs
        rho0 = self.model_rho(net_rho, [zic, 0.0*xic, xic])
        init = [zic, 0.0*xic, xic, vic]
    ...
    def model_rho(self, net_rho, inputs):
        z, t, x = inputs
        rho = t * net_rho(torch.cat([z, t, x], -1))
        return rho

    # inputs: (t, x, v)
    def model_r(self, net_r, inputs):
        z, t, x, v = inputs
        r_1 = net_r(torch.cat([z, t, x, v], -1))
        r_2 = net_r(torch.cat([z, t, x, -v], -1))
        return torch.exp(-0.5*(r_1 + r_2))

    # inputs: (t, x, v)
    def model_j(self, net_j, inputs):
        z, t, x, v = inputs
        j_1 = net_j(torch.cat([z, t, x, v], -1))
        j_2 = net_j(torch.cat([z, t, x, -v], -1))
        j = j_1 - j_2
        return j
    ...
    def val(self, net_rho, ref):
        ...
        ref_z = self.zmin + torch.rand((self.nx, self.num_sim, self.uq_dimension)).to(self.device) * (self.zmax - self.zmin)
        ref_t = self.ref_t[:,None,:] * torch.ones((ref_z.shape[0], ref_z.shape[1], 1)).to(self.device)
        ref_x = self.ref_x[:,None,:] * torch.ones((ref_z.shape[0], ref_z.shape[1], 1)).to(self.device)

        ref_ztx = torch.cat([ref_z, ref_t, ref_x], -1)
        model_rho = ref_t * net_rho(ref_ztx)
        
        density_approx = torch.mean(model_rho.to("cpu"), dim=-2) # shape: (self.nx, 1)
