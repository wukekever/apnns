...
class Sampler(object):
    ...
    def interior(self):
        ...
        v = self.v_range[0] + torch.rand((self.interior_samples, 1)).to(self.device) * (
    ...
    def boundary(self):
        ...
        v = torch.rand((self.boundary_samples, 1)).to(self.device) * self.v_range[-1]
        return (t, v)

    def initial(self):
        x = self.x_range[0] + torch.rand((self.initial_samples, 1)).to(self.device) * (
            self.x_range[-1] - self.x_range[0]
        )
        v = self.v_range[0] + torch.rand((self.initial_samples, 1)).to(self.device) * (
            self.v_range[-1] - self.v_range[0]
        )
        return (x, v)
